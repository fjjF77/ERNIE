...
if __name__ == "__main__":
    ...
    training_args.pipeline_parallel_degree = training_args.pipeline_degree
    training_args.tensor_parallel_degree = training_args.tensor_degree
    training_args.seed = 42

    if training_args.gradient_accumulation_steps == -1:
        logger.info("gradient_accumulation_steps will estimate automaically.")
    if paddle.distributed.get_world_size() > 1:
        raise NotImplementedError("DPO estimate training does not support multi-node.")
    ...
    is_local = os.path.isfile(model_args.model_name_or_path) or os.path.isdir(model_args.model_name_or_path)
    if getattr(model_args,"from_aistudio",False):
        if not is_local and model_args.model_name_or_path.startswith("baidu"):
            model_args.model_name_or_path = model_args.model_name_or_path.replace("baidu","PaddlePaddle")
            logger.warning(f"The repo id of baidu's model in the aistudio should be 'PaddlePaddle', model_name_or_path has changed to {model_args.model_name_or_path}")
    elif getattr(model_args,"from_modelscope",False):
        os.environ["from_modelscope"] = "True"
        model_args.model_name_or_path = model_args.model_name_or_path.replace("baidu","PaddlePaddle")
        logger.warning(f"The repo id of baidu's model in the modelscope should be 'PaddlePaddle', model_name_or_path has changed to {model_args.model_name_or_path}")
